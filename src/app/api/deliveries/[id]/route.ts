...
// GET single delivery; PATCH — assign driver (admin) / advance status (admin & driver)
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const delivery = await prisma.delivery.findUnique({
    where: { id },
    include: {
      orders: { include: { order: { include: { items: { include: { produceItem: true } }, subscription: { include: { user: { select: { name: true, phone: true } } } } } } } },
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!existingDelivery) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const isAdmin = user.role === "ADMIN";
  const isAssignedDriver = existingDelivery.driverId === user.id;
  if (!isAdmin && (!isAssignedDriver || user.role !== "DELIVERY")) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

...
// GET /api/deliveries — list deliveries (admin sees all, rider sees assigned)
export async function GET() {
  const user = await getSessionUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const where = user.role === "DELIVERY" ? { driverId: user.id } : {};
  const deliveries = await prisma.delivery.findMany({
    where,
    include: {
      orders: { include: { order: { include: { items: { include: { produceItem: true } } } } } },
